---
title: how2heap - house of orange
categories:
  - how2heap
tags: null
published: true
...
首先有一块放在 unsorted bin 的 chunk，模拟溢出或者 uaf 漏洞构造 _IO_FILE_plus 变量需要的值，
 `fp->_mode <= 0 && fp->_IO_write_ptr > fp->_IO_write_base` 和 jump table。

`_IO_FILE_plus` (glibc-2.23/libio/libioP.h):
...
`_IO_FILE` (glibc-2.23/libio/libio.h):
...
![house_of_orange]({{site.baseurl}}/images/house_of_orange.png)
